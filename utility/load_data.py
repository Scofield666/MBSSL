...
class DataSimChecker(DataHandler):
    ...
    def unified_sim(self):
        ...
        for beh in range(1, len(self.behs)):
            ...
            # del user_sim_mat, item_sim_mat
        return user_sim_mat_unified, item_sim_mat_unified

    def save_unified_sim(self):
        t1 = time()
        user_sim_mat, item_sim_mat = self.unified_sim()
        user_unified_sim_file_name = "_".join(["user_unified_sim_mat", self.sim_measure, ".npz"])
        item_unified_sim_file_name = "_".join(["item_unified_sim_mat", self.sim_measure, ".npz"])
        sp.save_npz(os.path.join(self.saveSimMatPath, user_unified_sim_file_name), user_sim_mat)
        sp.save_npz(os.path.join(self.saveSimMatPath, item_unified_sim_file_name), item_sim_mat)
        print('Save unified Sim: ', time() - t1)

    def preprocess_sim_v3(self):
        t1 = time()
        input_u_sim, input_i_sim = self.unified_sim()
        input_u_sim = torch.tensor(input_u_sim.todense())
        input_i_sim = torch.tensor(input_i_sim.todense())
...
#     args = parser.parse_args()
    ...
    # 生成unified_sim_mat: python utility/load_data.py --dataset Beibei --sim_weights [0.,0.,1.]
    # simchecker = DataSimChecker(args, args.dataset, args.batch_size)
    # simchecker.save_unified_sim()
